...
# ==================== YOUR COMPLETE COURSES DATA (CONVERTED TO PYTHON DICTIONARY) ====================
coursesData = {
  1: {
      ...
      3: {
        ...
        "semesters": {
          5: [
            ...
            {"id": 236, "name": "Cryptography", "code": "TBC508", "credits": 3, "materials": 15, "rating": 4.4, "type": "Theory"},
...
def create_folder_structure(base_path="uploads"):
    """Create the complete folder structure based on coursesData"""
    
    print("=" * 60)
    print("🚀 CREATING FOLDER STRUCTURE FOR NOTES HUB")
    print("=" * 60)
    ...
    # Material type folders to create under each subject
    material_folders = [
        "notes",
        "syllabus",
        "pyq",
    ...
    # Course ID to folder name mapping
    course_folder_map = {
        1: "BTECH",
        2: "BCA",
        3: "BBA",
    ...
    for course_id, course_data in coursesData.items():
        course_name = course_data.get("name", f"Course_{course_id}")
        course_folder_name = course_folder_map.get(course_id, clean_folder_name(course_name))
        
        print(f"\n📚 Processing: {course_name} -> {course_folder_name}")
        
        # Create course folder
        course_path = base_dir / course_folder_name
        course_path.mkdir(exist_ok=True)
        print(f"  📁 Created: {course_folder_name}/")
        
        # Loop through years
        years = course_data.get("years", {})
        for year_num, year_data in years.items():
            year_folder = f"Year{year_num}"
            year_path = course_path / year_folder
            year_path.mkdir(exist_ok=True)
            print(f"    📁 Created: {course_folder_name}/{year_folder}/")
            
            # Loop through semesters
            semesters = year_data.get("semesters", {})
            for sem_num, subjects in semesters.items():
                sem_folder = f"Sem{sem_num}"
                sem_path = year_path / sem_folder
                sem_path.mkdir(exist_ok=True)
                print(f"      📁 Created: {course_folder_name}/{year_folder}/{sem_folder}/")
                
                # Loop through subjects in this semester
                for subject in subjects:
                    subject_name = subject.get("name", "Unknown_Subject")
                    subject_code = subject.get("code", "")
                    
                    # Create subject folder name with code
                    subject_folder_name = clean_folder_name(f"{subject_name}_{subject_code}")
                    subject_path = sem_path / subject_folder_name
                    subject_path.mkdir(exist_ok=True)
                    
                    # Create material type folders under each subject
                    for material_folder in material_folders:
                        material_path = subject_path / material_folder
                        material_path.mkdir(exist_ok=True)
                        total_folders += 1
                    
                    # Print progress occasionally
                    if total_folders % 50 == 0:
                        print(f"        ... created {total_folders} folders so far")
    
    print("\n" + "=" * 60)
    print(f"✅ COMPLETE! Total folders created: {total_folders}")
    print("=" * 60)
    ...
    print("uploads/")
    for course_id, course_data in coursesData.items():
        course_folder = course_folder_map.get(course_id, "OTHER")
        print(f"  ├── {course_folder}/")
        
        years = course_data.get("years", {})
        for year_num in years.keys():
            print(f"  │   ├── Year{year_num}/")
            semesters = years[year_num].get("semesters", {})
            for sem_num in semesters.keys():
                print(f"  │   │   ├── Sem{sem_num}/")
                subjects = semesters[sem_num]
                # Show first 2 subjects as example
                for i, subject in enumerate(subjects[:2]):
                    subject_name = subject.get('name', 'Subject')[:30] + "..."
                    print(f"  │   │   │   ├── {subject_name}_{subject.get('code', '')}/")
                    print(f"  │   │   │   │   ├── notes/")
                    print(f"  │   │   │   │   ├── syllabus/")
                    print(f"  │   │   │   │   ├── pyq/")
                    print(f"  │   │   │   │   └── ...")
                if len(subjects) > 2:
                    print(f"  │   │   │   └── ... ({len(subjects)-2} more subjects)")
    
    print(f"\n📁 Total courses: {len(coursesData)}")
    print(f"📁 Total folders created: {total_folders}")
    print(f"📁 Base location: {base_dir.absolute()}")
...
if __name__ == "__main__":
    try:
        ...
        create_folder_structure("uploads")
        
        print("\n✨ To create in different location, run:")
        print('   create_folder_structure("path/to/your/uploads")')
